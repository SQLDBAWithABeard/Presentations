...
# Lets take a look at the Agent Jobs on SQL0
 Get-DbaAgentJob -SqlInstance $sql0 | Format-Table

#And the Agent Jobs on SQL1
 Get-DbaAgentJob -SqlInstance $sql1 | Format-Table

 Get-DbaAgentJobCategory -SqlInstance $sql0 |Format-Table
 Get-DbaAgentJobCategory -SqlInstance $sql1 |Format-Table

 Get-DbaAgentOperator -SqlInstance $sql0 |Format-Table
 Get-DbaAgentOperator -SqlInstance $sql1 |Format-Table

 # Lets copy them over

 Copy-DbaAgentJobCategory -Source $sql0 -Destination $sql1
 Copy-DbaAgentJob -Source $sql0 -Destination $sql1
 Copy-DbaAgentOperator -Source $sql0 -Destination $sql1
...
Explorer $BackupWin
Get-DbaDatabase -SqlInstance $sql0 -ExcludeAllSystemDb  | Backup-DbaDatabase -BackupDirectory $BackupLinux -Type FULL -CopyOnly -CreateFolder
...
Invoke-Item C:\temp\Excel\ThroughputAutoSize.xlsx 
...
Invoke-Item C:\temp\Excel\ThroughputConditional.xlsx
#endregion
...
$Path = (Get-ChildItem $BackupWin\KEEP\NOrthWind\* | Sort-Object LastWriteTime -Descending)[0].FullName

Get-DbaFile -Path /var/opt/mssql/backups/NOrthWind -SqlInstance $sql1 

$path = '/var/opt/mssql/backups/NOrthWind\Northwind_201910181245.bak'
...
Measure-DbaDiskSpaceRequirement -Source $SQL0 -Destination $sql1 -Database NOrthWind
#endregion

## But back to the disaster!
# Restore a file
Restore-DbaDatabase -SqlInstance $sql1 -Path $path -WithReplace
## restore databases from backup folder
Restore-DbaDatabase -SqlInstance $sql1 -Path $BackupLinux -WithReplace
...
Get-DbaDatabase -SqlInstance $sql1 -ExcludeAllSystemDb -ExcludeDatabase WideWorldImporters, ValidationResults | Backup-DbaDatabase -BackupDirectory $BackupLinux\QuickBackup
...
Invoke-Item '.\Test Your Backups.mp4'
#endregion
...
(Get-DbaAgentJobOutputFile -SqlInstance $SQL0 -Job 'DatabaseBackup - USER_DATABASES - FULL' ).RemoteOutputFileName
...
## Run the job
(Get-DbaAgentJob -SqlInstance localhost -SqlCredential $cred -Job 'DatabaseBackup - USER_DATABASES - FULL').start()
Get-DbaAgentJob -SqlInstance localhost -SqlCredential $cred | Format-Table

$scriptBlock = {
    $x = 0
    while ($x -lt 5) {
        (Get-DbaAgentJob -SqlInstance localhost -SqlCredential $Using:cred -Job 'DatabaseBackup - USER_DATABASES - DIFF').start()
        $y = 0
        while ($y -lt 5) {
            (Get-DbaAgentJob -SqlInstance localhost -SqlCredential $Using:cred -Job 'DatabaseBackup - USER_DATABASES - LOG').start()
            $y ++  
...
## Check the history
Get-DbaAgentJobHistory -SqlInstance localhost -Job 'DatabaseBackup - USER_DATABASES - DIFF'

## Check the backup history
## Answer the question - When was this database backed up

Get-DbaDbBackupHistory -SqlInstance localhost  # other params -Since -Last -LastFull -LastDiff -LastLog -LastLsn 
...
Get-DbaDbRestoreHistory -SqlInstance $sql1 
...
Test-DbaPath -SqlInstance $SQL0 -Path $BackupLinux
...
Get-DbaFile -SqlInstance $sql1 -Path '/var/opt/mssql/data/'
Get-DbaFile -SqlInstance $sql1 -Path '/var/opt/mssql/'
Get-DbaFile -SqlInstance $sql1 -Path '/var/opt/mssql/log'

## and create a directory

New-DbaDirectory -SqlInstance localhost -Path C:\MSSQL\BACKUP\Beard

## Oh and dbatools can restore from a Ola Hallengren directory too

## Check the job is not running
Get-DbaAgentJob -SqlInstance localhost | Format-Table
Get-Job -Name DoSomeBackupsPlease -IncludeChildJob | Select JobStateInfo 

## show the databases
Get-DbaDatabase -SqlInstance localhost -ExcludeAllSystemDb  |Format-Table
...
Get-DbaDatabase -SqlInstance $sql1 -ExcludeAllSystemDb 

## Restore from Ola directory
Restore-DbaDatabase -SqlInstance $sql1 -Path '/var/opt/mssql/data/BeardLinuxSQL' -AllowContinue -Verbose

## show the databases
Get-DbaDatabase -SqlInstance $sql1 -ExcludeAllSystemDb | Format-Table
...
# See protocols
Get-DbaClientProtocol -ComputerName $sql0
Get-DbaInstanceProtocol -ComputerName $sql0
...
Get-DbaOperatingSystem -ComputerName localhost
