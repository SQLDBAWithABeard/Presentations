...
$SQLInstances = $containers = $dbatools1, $dbatools2 , $dbatools3 = 'localhost,11001','localhost,11002','localhost,11003'

[pscredential]$cred = New-Object System.Management.Automation.PSCredential (Get-Secret -Name sqladmin -Vault SecretStore -AsPlainText), (Get-Secret -Name sqladminpwd -Vault SecretStore)
...
$CheckResults = Invoke-DbcCheck -Check Agent, Instance, Database -Show $show -legacy $false -PassThru
...
$sqlinstance = 'mahomes.database.windows.net'
$database = 'dbachecks'

$mahomescred = New-Object -TypeName System.Management.Automation.PSCredential -ArgumentList "sqladmin",  (Get-Secret -Name mahomescred -Vault LocalStore)

$CheckResults | Convert-DbcResult -Label 'dev' | Write-DbcTable -SqlInstance $sqlinstance -SqlCredential $mahomescred -Database $database

$CheckResults = Invoke-DbcCheck -Check Instance, Database -Show $show -legacy $false -PassThru

$CheckResults | Convert-DbcResult -Label 'DatabaseInstance' | Write-DbcTable -SqlInstance $sqlinstance -SqlCredential $mahomescred -Database $database
