...
sqlcmd create mssql --accept-eula --cached --user-database dontdelete
...
# run a query against the dataweekender database

sqlcmd query "SELECT DB_NAME() AS [Current Database]" --database dontdelete

# run a query file against the dataweekender database

sqlcmd --input-file 'dataweekender.sql' --database-name dontdelete

# what do we have?

sqlcmd query "SELECT Name FROM sys.tables" --database dontdelete

# the sort of things to make you go hmmmm

sqlcmd query "SELECT  COLUMN_NAME,DATA_TYPE FROM INFORMATION_SCHEMA.COLUMNS WHERE TABLE_NAME = N'table holding data with a bad name'" --database dontdelete
sqlcmd --query "SELECT  COLUMN_NAME,DATA_TYPE FROM INFORMATION_SCHEMA.COLUMNS WHERE TABLE_NAME = N'table holding data with a bad name'" --database-name dontdelete --format="vert"
...
:setvar SQLCMDCOLORSCHEME rainbow_dash

SELECT  COLUMN_NAME,DATA_TYPE FROM INFORMATION_SCHEMA.COLUMNS WHERE TABLE_NAME = N'table holding data with a bad name'
GO
#>

# so that was the container with the dataweekender database. What about the other one?
...
sqlcmd create mssql --accept-eula  --cached --using 'https://beardsqlbaks1.blob.core.windows.net/backups/someonlinedeebeebackup.bak?sp=racwd&st=2023-09-12T15:31:05Z&se=2023-09-17T23:31:05Z&spr=https&sv=2022-11-02&sr=b&sig=FRda2aEhH6aBk2rNLmtQ4zcJbi8ykWHCWM4OaSGDlpQ%3D' --verbosity 5
...
./sqlcmd use C:\Backups\AdventureWorks2017.bak
...
./sqlcmd use C:\Backups\AdventureWorks2017.bak,thatdatabasename
...
./sqlcmd create mssql --cached --use C:\Backups\AdventureWorks2017.bak,anevenlongerdatabasenametoseeifanyoneisreadingthissaybeard --open ads
