...
$passwordstring = "1l8C4k&W!z!8WjD5k*V9x6R8%BU3&ErONd@DAI!7eE$%E!s7&6" (($connectionstrings -split "SQLCMDPASSWORD=")[2] -split '"')[0]
...
sqlcmd create mssql --accept-eula --cached --user-database ScotlandScotlandScotland
...
# run a query against the ScotlandScotlandScotland database

sqlcmd query "SELECT DB_NAME() AS [Current Database]" --database ScotlandScotlandScotland

# run a query file against the ScotlandScotlandScotland database

sqlcmd --input-file 'killswitchengage.sql' --database-name ScotlandScotlandScotland

# what do we have?

sqlcmd query "SELECT Name FROM sys.tables" --database ScotlandScotlandScotland

# the sort of things to make you go hmmmm

sqlcmd query "SELECT  COLUMN_NAME,DATA_TYPE FROM INFORMATION_SCHEMA.COLUMNS WHERE TABLE_NAME = N'table holding data with a bad name'" --database ScotlandScotlandScotland
sqlcmd --query "SELECT  COLUMN_NAME,DATA_TYPE FROM INFORMATION_SCHEMA.COLUMNS WHERE TABLE_NAME = N'table holding data with a bad name'" --database-name ScotlandScotlandScotland --format="vert"
...
# so that was the container with the ScotlandScotlandScotland database. What about the other one?
