...
@minValue(1)
@maxValue(64)
@description('The number of data disks - default 1')
param dataDisksCount int = 1

@minValue(16)
@maxValue(4096)
@description('Size of the data disks in GB - default 128')
param DataDisksSizeGb int = 100
...
resource windowsVM 'Microsoft.Compute/virtualMachines@2020-12-01' = {
  ...
  properties: {
    ...
    storageProfile: {
      ...
      osDisk: {
        name: '${name}-${osDiskName}'
        caching: osDiskCaching
        createOption: osDiskCreateOption
      }
      dataDisks:  [for diskno in range(0, dataDisksCount): {
        caching: 'ReadWrite'
        diskSizeGB: DataDisksSizeGb
        lun: diskno
        name: '${name}-datadisk${diskno}'
        createOption: 'Empty'
      }]
    }
    networkProfile: {
