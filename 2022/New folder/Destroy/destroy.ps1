$_names = 'AzureBeard', 'AzureBeards', 'AzureBeard-Admin','NewBeard-Admin', 'beardednetwork-rg', 'beardynetwork-dev-rg', 'beardynetwork-uat-rg', 'beardynetwork-rg','demo-rg','beard-admin-rg'
...
$_names | ForEach-Object -Parallel  {
    $r = Get-AzResourceGroup -Name $_ -ErrorAction SilentlyContinue
    if ($r) {
        $message = "We have a resource group called {0}" -f $_
        Write-PSFMessage $message -Level Output
